...
// Singleton instance
  ...
  static const String _referredByUserIdKey = 'referred_by_user_id';
  static const String _referredByCodeKey = 'referred_by_code';
  static const String _referralRegisteredAtKey = 'referral_registered_at';
  ...
  void loadValueHolder() {
    ...
    final String _defaultCurrency =
        shared.getString(PsConst.VALUE_HOLDER__DEFAULT_CURRENCY) ?? 'L.E.';
    final String _defaultCurrencyId =
        shared.getString(PsConst.VALUE_HOLDER__DEFAULT_CURRENCY_ID) ?? '1';
    ...
    final String _userGender =
        shared.getString('user_gender') ?? '';
    final String _userAgeRange =
        shared.getString('user_age_range') ?? '';
    final bool _hasFavCategories =
        shared.getBool(PsConst.VALUE_HOLDER__HAS_FAV_CATEGORIES) ?? false;

    final String _referralCode = shared.getString(_referralCodeKey) ?? '';
    final String _referredByUserId = shared.getString(_referredByUserIdKey) ?? '';
    final String _referredByCode = shared.getString(_referredByCodeKey) ?? '';
    final String _referralRegisteredAt =
        shared.getString(_referralRegisteredAtKey) ?? '';
    final String _pendingReferralCode =
        shared.getString(_pendingReferralCodeKey) ?? '';
  ...
  Future<dynamic> replaceHasFavCategories(bool value) async {
    await shared.setBool(PsConst.VALUE_HOLDER__HAS_FAV_CATEGORIES, value);
    loadValueHolder();
  }

  bool getHasFavCategories() {
    return shared.getBool(PsConst.VALUE_HOLDER__HAS_FAV_CATEGORIES) ?? false;
  }
  ...
  Future<dynamic> replaceReferralCode(String referralCode) async {
    await shared.setString(_referralCodeKey, referralCode);
    loadValueHolder();
  }

  Future<dynamic> replaceReferralData({
    String? referralCode,
    ...
    if (referredByUserId != null) {
      await shared.setString(_referredByUserIdKey, referredByUserId);
    }
    if (referredByCode != null) {
      await shared.setString(_referredByCodeKey, referredByCode);
    }
    if (referralRegisteredAt != null) {
      await shared.setString(_referralRegisteredAtKey, referralRegisteredAt);
    }
    loadValueHolder();
  }

  Future<dynamic> savePendingReferralCode(String referralCode) async {
    final String clean = referralCode.trim();
    if (clean.isEmpty || clean.toLowerCase() == 'null') return;

    final String currentUserReferral =
        shared.getString(_referralCodeKey)?.trim() ?? '';

    // Do not keep self-referrals.
    if (currentUserReferral.isNotEmpty && currentUserReferral == clean) return;

    await shared.setString(_pendingReferralCodeKey, clean);
    loadValueHolder();
  }

  String getPendingReferralCode() {
    return shared.getString(_pendingReferralCodeKey) ?? '';
  }

  Future<dynamic> clearPendingReferralCode() async {
    await shared.remove(_pendingReferralCodeKey);
    loadValueHolder();
  }

  String getReferralCode() {
    return shared.getString(_referralCodeKey) ?? '';
  }
  ...
  Future<dynamic> replaceUserGender(String value) async {
    await shared.setString('user_gender', value);
    loadValueHolder();
  }

  Future<dynamic> replaceUserAgeRange(String value) async {
    await shared.setString('user_age_range', value);
    loadValueHolder();
  }
  ...
  String getLoggedUserId() {
    return shared.getString(PsConst.VALUE_HOLDER__USER_ID) ?? '';
  }
}
