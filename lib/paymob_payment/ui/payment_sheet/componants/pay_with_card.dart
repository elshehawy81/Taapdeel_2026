...
dynamic payWithCardButton(BuildContext context, SubscriptionPackage subscriptionPackage,
    {BillingData? billingData, bool fromPromoteScreen =false , dynamic  promoteFunction}) {
        ...
        if(fromPromoteScreen ==false){
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              content: Text(
                response.message ??
                    "${subscriptionPackage.swapRequests} "+"requests_added".tr(),
                style: TextStyle(color: Colors.black,fontSize: 20.sp),
              ),
            ),
          );
