...
class SearchProvider extends ChangeNotifier {
  ...
  // Constants
  // =============================
  static const int _defaultTimeoutSeconds = 30;       // ✅ FIX: raised from 15
  static const int _networkItemsTimeoutSeconds = 45;  // ✅ FIX: raised from 20
  ...
  List<String> _preferredSubCatIdsFromShared() {
    try {
      final String jsonString =
          PsSharedPreferences.instance.shared.getString('myListKey') ?? '';

      if (jsonString.trim().isEmpty) {
        tlog('PREFCAT_SHARED', 'myListKey is empty');
        return <String>[];
      }

      final List<SubCategory> selected =
      SubCategory().fromMapList(json.decode(jsonString));
      ...
      tlog(
        'PREFCAT_SHARED',
        'myListKey count=${selected.length} parsedIds=${ids.join(",")}',
      );
  ...
  Future<List<String>> _preferredSubCatIdsFromServer(String userId) async {
    ...
    try {
      // ✅ FIX: استخدام _client المشترك بدل client جديد
      ...
      final List<SubCategory> serverList = SubCategory().fromMapList(decoded);

      final List<String> ids = serverList
          .map((e) => (e.id ?? '').trim())
          .where((id) => id.isNotEmpty)
          .toSet()
          .toList();
      ...
      tlog(
        'PREFCAT_SERVER',
        'userId=$userId count=${serverList.length} parsedIds=${ids.join(",")}',
      );
      ...
      tlog(
        'PREFCAT_SERVER_ERR',
        'Failed to fetch preferred subcategories from server',
        err: e,
        st: st,
      );
      return <String>[];
  ...
  Future<List<String>> _preferredSubCatIds() async {
    // ✅ FIX: cache hit — لا نطلب السيرفر مرة ثانية
    if (_cachedPrefCatIds != null) {
      tlog('PREFCAT_CACHE', 'Using cached prefcat IDs: ${_cachedPrefCatIds!.join(",")}');
      return _cachedPrefCatIds!;
    }

    final String userId = _myUserId();

    List<String> result;

    if (userId.isNotEmpty) {
      final List<String> fromServer = await _preferredSubCatIdsFromServer(userId);
      if (fromServer.isNotEmpty) {
        result = fromServer;
      } else {
        final List<String> fromShared = _preferredSubCatIdsFromShared();
        if (fromShared.isNotEmpty) {
          tlog('PREFCAT_FALLBACK', 'Using shared fallback after server returned empty');
        }
        result = fromShared;
      }
    } else {
      result = _preferredSubCatIdsFromShared();
    }

    _cachedPrefCatIds = result;
    return result;
  }
  ...
  Future<void> getFilteredProducts({
    ...
    try {
      ...
      if (_isPrefcatBulk(filterUrl)) {
        hasMore = false;
      } else if (firstPage.length < _pageSize) {
        hasMore = false;
      } else {
        _offset += _pageSize;
      }
  ...
  Future<void> loadMoreFilteredProducts({
    ...
    if (!hasMore) return;
    if (_isPrefcatBulk(urlToUse)) return;
  ...
  Future<void> loadSection({
    ...
    try {
      ...
      _rebuildSubCats(filterUrl); // ✅ بناء الـ sub-cats من المنتجات
      ...
      _sectionProducts[filterUrl] = <Product>[];
      _sectionHasMore[filterUrl] = false;
  ...
  Future<void> loadMoreSection({
    ...
    if (_isPrefcatBulk(filterUrl)) return;
  ...
  void preloadOtherSections({
    ...
    for (final url in urlsToPreload) {
      ...
      // ✅ FIX: بدون delay — فوري في الخلفية
  ...
  bool _isPrefcatBulk(String filterUrl) {
    return filterUrl == PsUrl.ps_Prefcat_bulk_url;
  }
  ...
  Future<List<Product>> _fetchProducts({
    ...
    final isPrefcat = _isPrefcatBulk(filterUrl);

    final String url = isPrefcat
        ? '$appUrl$filterUrl'
        : isFF
        ? '$appUrl$filterUrl/api_key/$apiKey'
        : '$appUrl$filterUrl/api_key/$apiKey/limit/$limit/offset/$offset/login_user_id/$loginUserId';
    ...
    if (isPrefcat) {
      final prefSubIds = await _preferredSubCatIds();

      if (prefSubIds.isEmpty) {
        tlog('PREFCAT_EMPTY', 'No preferred subcategories found from server/shared');
        return <Product>[];
      }

      body.remove('cat_id');
      body['added_user_id'] = loginUserId;
      body['limit'] = limit.toString();
      body['sub_cat_ids'] = prefSubIds.join(',');
    }
    ...
    tlog(
      'NET_REQ',
      '[$reqId] POST url=$url\n'
          'filterUrl=$filterUrl isFF=$isFF isPrefcat=$isPrefcat\n'
    ...
    tlog(
      'NET_RES',
      '[$reqId] status=${response.statusCode} time=${sw.elapsedMilliseconds}ms url=$url\n'
