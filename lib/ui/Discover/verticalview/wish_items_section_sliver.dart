...
class _WishItemsSectionSliverState extends State<WishItemsSectionSliver> {
    ...
    return SliverToBoxAdapter(
      child: Selector<SearchProvider, _WishSectionData>(
        ...
        builder: (ctx, data, _) {
          ...
          // kick load-more when near end
          if (!loading && !loadingMore) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (!context.mounted) return;
              // We only ask for more if 80 % of current items are visible —
              // but since we're in a non-scrollable column, trigger when fully rendered.
              if (raw.length < _pageSize) return; // not enough to page yet
              final sp = SearchProvider.of(context, listen: false);
              _maybeLoadMore(sp, raw, loading, loadingMore);
            });
          }
              ...
              if (!loading && filtered.isEmpty) ...[
                ...
                ...withStory.asMap().entries.map((entry) {
                  ...
                  return HawadeetWishCard(
                    ...
                    onHaveItem: () => widget.onAddWishItem(),
                    onShare: () {
  ...
  void _navigateToOffer(BuildContext ctx, WishCardData m) {
    // TODO: open item-entry for creating an offer swap against this wish
    // Placeholder: navigate to general add item screen
    widget.onAddWishItem();
  }
}
