...
class _HomeViewState extends State<DashboardView>
  ...
  bool _handledInitialDashboardRouteArgument = false;
  String _lastContactNetworkBootUid = '';

  // Forces the swap-opportunities tab to rebuild from scratch after a product
  // is uploaded. This guarantees the tab reloads its data instead of showing
  // the old cached widget state.
  int _swapOpportunitiesReloadNonce = 0;
  ...
  bool _fcmConfigureStarted = false;
  ...
  void reloadSwapOpportunitiesAfterProductUpload(String itemId) {
    _clearHomeSearchFocus();

    final dynamic returnValue = getIndexFromBottonNavigationIndex(1);

    if (!mounted) return;
    setState(() {
      _itemId = itemId;
      appBarTitleName = '';
      appBarTitle = returnValue[0] as String?;
      _currentIndex = returnValue[1] as int?;
      _swapOpportunitiesReloadNonce++;
    });
  }

  void reloadSwapOpportunities() {
    if (!mounted) return;
    setState(() {
      _swapOpportunitiesReloadNonce++;
    });
  ...
  void refreshSwapOpportunities() => reloadSwapOpportunities();

  void refreshSuggestedSwaps() => reloadSwapOpportunities();

  Future<FirebaseMessaging?> _ensureFirebaseMessagingReady() async {
    try {
      ...
      debugPrint('DASHBOARD_FCM_INIT_ERROR=${e.code}: ${e.message}');
    } catch (e) {
      debugPrint('DASHBOARD_FCM_INIT_ERROR=$e');
    }
    return null;
  }

  Future<void> _saveDeviceTokenSafely(NotificationProvider provider) async {
    final FirebaseMessaging? fcm = await _ensureFirebaseMessagingReady();
    if (fcm == null || !mounted) return;

    try {
      Utils.saveDeviceToken(fcm, provider);
    } catch (e) {
      debugPrint('DASHBOARD_SAVE_DEVICE_TOKEN_ERROR=$e');
    }
  }

  Future<void> _loadSweetBadgeSafely(
    BuildContext context,
    String uid,
  ) async {
    TaapdeelPerfBenchmark.start('sweet_badge_load');
    try {
      await context
      ...
      debugPrint('DASHBOARD_SWEET_BADGE_LOAD_ERROR=$e');
    } finally {
      TaapdeelPerfBenchmark.end('sweet_badge_load');
    }
  }

  Future<void> _loadFollowBadgeSafely(
    BuildContext context,
    String uid,
  ) async {
    TaapdeelPerfBenchmark.start('follow_badge_load');
    try {
      await context
      ...
      debugPrint('DASHBOARD_FOLLOW_BADGE_LOAD_ERROR=$e');
    } finally {
      TaapdeelPerfBenchmark.end('follow_badge_load');
    }
  }

  Future<void> _loadAddProductCoachCountSafely(
    BuildContext context,
    String uid,
  ) async {
    try {
      final AddedItemProvider addedProvider =
          context.read<AddedItemProvider>();
      addedProvider.addedUserParameterHolder.mile = psValueHolder!.mile;
      addedProvider.addedUserParameterHolder.addedUserId = uid;
      addedProvider.addedUserParameterHolder.status = '1';
      await addedProvider
          .loadItemList(uid, addedProvider.addedUserParameterHolder)
          .timeout(const Duration(seconds: 8));
    } catch (e) {
      debugPrint('DASHBOARD_ADD_PRODUCT_COACH_COUNT_ERROR=$e');
    }
  }
  ...
  void _initFcmOnceAfterFirstBuild() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      if (!isFirstTime || _fcmConfigureStarted) return;

      _fcmConfigureStarted = true;
      ...
      try {
        ...
        Utils.fcmConfigure(context, fcm, valueHolder!.loginUserId, () {
        ...
        _fcmConfigureStarted = false;
        debugPrint('DASHBOARD_FCM_CONFIGURE_ERROR=$e');
      }
    });
  }
  ...
  Future<void> _bootstrapAddProductCoach() async {
    ...
    _addProductCoachTimer = Timer(const Duration(seconds: 5), () {
  ...
  Future<void> updateSelectedIndexWithAnimation(String? title, int? index) async {
    ...
    // ✅ BENCHMARK: وقت تبديل التاب (animation + setState)
    TaapdeelPerfBenchmark.start('tab_switch_$index');
    ...
    await animationController.reverse().then<dynamic>((void data) {
      ...
      // ✅ BENCHMARK: tab switch كامل انتهى
      TaapdeelPerfBenchmark.end('tab_switch_$index');
    });
  }
  ...
  bool _hasValidLoginUser() {
    final String uid = (valueHolder?.loginUserId ?? psValueHolder?.loginUserId ?? '')
        .trim()
        .toLowerCase();
  ...
  Future<void> _handleAddProductButtonPressed() async {
    if (_isAddSheetOpening) return;
    _isAddSheetOpening = true;

    await _dismissAddProductCoach();

    if (!mounted) {
      _isAddSheetOpening = false;
      return;
    }

    if (!_hasValidLoginUser()) {
      _isAddSheetOpening = false;
      _goToLoginTab();
      return;
    }
  ...
  void initState() {
    super.initState();
    // ✅ BENCHMARK: وقت تحميل الداشبورد كاملاً من initState لأول frame
    TaapdeelPerfBenchmark.start('dashboard_boot');
  ...
  void _showAddBottomSheet(BuildContext context) {
    DashboardSheets.showAddBottomSheet(
      context: context,
      onAddProduct: _openSingleProductEntry,
      onAddBulkProducts: _openBulkProductEntry,
    );
  }

  void _openSingleProductEntry() {
    if (!_hasValidLoginUser()) {
      _goToLoginTab();
      return;
    }
  ...
  void didChangeDependencies() {
    ...
    _handleInitialDashboardRouteArgument();
  }

  void _handleInitialDashboardRouteArgument() {
    if (_handledInitialDashboardRouteArgument) return;

    final Object? argument = ModalRoute.of(context)?.settings.arguments;
    final int? requestedIndex = argument is int ? argument : null;

    if (requestedIndex == null) return;

    _handledInitialDashboardRouteArgument = true;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        appBarTitle = requestedIndex == PsConst.REQUEST_CODE__DASHBOARD_NOTI_FRAGMENT
            ? Utils.getString(context, 'my_requests')
            : appBarTitle;
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
        providers: [
          ...
          // ✅ Used only to decide if "ضيف أول منتج" should appear.
          // PERF: do not load it immediately on Dashboard creation. HomeProvider
          // already loads My Products for ForYou. This provider is delayed below
          // so first open does not send a duplicate My Products request.
          ChangeNotifierProvider<AddedItemProvider>(
            lazy: false,
            create: (_) => AddedItemProvider(
        ...
        builder: (innerContext, child) {
          if (!_dashboardBackgroundBootstrapped) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              ...
              // ✅ BENCHMARK: أول frame ظهر — الداشبورد جاهز للاستخدام
              TaapdeelPerfBenchmark.end('dashboard_boot');
              ...
              // 2) Run login-only background tasks after the page is stable.
              // PERF: badges are profile indicators, not required for first ForYou paint.
              // Split their timings and give each a short timeout instead of one long
              // badges_load measurement that can run for tens of seconds.
              Future<void>.delayed(const Duration(seconds: 15), () {
                ...
                unawaited(_loadSweetBadgeSafely(innerContext, uid));
                unawaited(_loadFollowBadgeSafely(innerContext, uid));
              ...
              // 3) لا نحمل قائمة منتجات المستخدم هنا مرة ثانية.
              // HomeProvider هو مصدر الحقيقة في صفحة الترشيحات، وأي طلب إضافي هنا
              // كان يزاحم أول تحميل ويجعل الصفحة تظهر كأن المستخدم بلا منتجات.
            });
          }
          ...
          return WillPopScope(
            ...
            child: Scaffold(
              ...
              bottomNavigationBar: SafeArea(
                ...
                child: Consumer3<SweetMessageBadgeProvider,
                    FollowRequestBadgeProvider,
                    AddedItemProvider>(
                  builder: (context, badgeProvider, followBadge, addedProvider, _) {
                    ...
                    final String uid = (psValueHolder?.loginUserId ?? '').trim();
                    final bool isRealLoggedIn = uid.isNotEmpty && uid != 'nologinuser';
                    final bool productsCountReady =
                        addedProvider.itemList.status != PsStatus.NOACTION &&
                            addedProvider.itemList.status != PsStatus.BLOCK_LOADING &&
                            addedProvider.itemList.status != PsStatus.PROGRESS_LOADING &&
                            addedProvider.itemList.status != PsStatus.LOADING;
                    final int activeProductsCount =
                        addedProvider.itemList.data?.length ?? 0;
                    final bool shouldShowAddProductCoach =
                        _showAddProductCoach &&
                            (
                                !isRealLoggedIn ||
                                    (productsCountReady && activeProductsCount == 0)
                            );
                    ...
                    return DashboardBottomNav(
                      ...
                      showAddProductCoach: shouldShowAddProductCoach,
                      ...
                      onAddPressed: () {
                        unawaited(_handleAddProductButtonPressed());
                      },
                    );
                  },
                ),
              ...
              body: ChangeNotifierProvider<NotificationProvider>(
                ...
                create: (BuildContext context) {
                  ...
                  if (provider.psValueHolder!.deviceToken == null ||
                    ...
                    unawaited(_saveDeviceTokenSafely(provider));
                  }
                  return provider;
                },
                child: Builder(
                      ...
                      if (valueHolder!.loginUserId != null &&
                        ...
                        return MultiProvider(
                            ...
                            child: Consumer<UserProvider>(builder:
                              ...
                              return KeyedSubtree(
                                key: ValueKey<String>(
                                  'swap_opportunities_${_swapOpportunitiesReloadNonce}_${_itemId ?? ''}',
                                ),
