...
class AllControllerTextWidget extends StatefulWidget {
  ...
  final Product? item;
  final LatLng? latlng;
  final Future<bool> Function(String itemId)? uploadImage;

  /// أدق من isImageSelected: يرجع true لو فيه أي صورة فعلية في slots الشاشة
  /// سواء صورة جديدة، كاميرا، صورة مرفوعة سابقاً، أو فيديو.
  final bool Function()? hasAnyProductMedia;
...
class AllControllerTextWidgetState extends State<AllControllerTextWidget> {
  ...
  bool get _isNew {
    return _selectedConditionIndex == 0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget _buildPriceMinMaxFields() {
      ...
      final bool priceBlockCompleted = hasSelection;
  ...
  Future<void> _handleSubmit() async {
    ...
    final String computedItemPriceTypeId = provider.itemPriceTypeId ?? '';
    ...
    try {
      ...
      if (itemData.status == PsStatus.SUCCESS && itemData.data != null) {
        ...
        if (widget.uploadImage != null && provider.itemId != null) {
          // ✅ ننتظر رفع كل الصور/الفيديو بالكامل قبل إظهار شاشة مشاركة المنتج.
          final bool mediaUploaded = await widget.uploadImage!(provider.itemId!);
  ...
  bool _hasAnyProductMedia() {
    try {
      final bool? fromOwner = widget.hasAnyProductMedia?.call();
      if (fromOwner == true) return true;
    } catch (_) {}

    if (widget.isImageSelected.contains(true)) return true;
    if (widget.isSelectedVideoImagePath == true) return true;

    try {
      final dynamic galleryList = widget.galleryProvider?.galleryList.data;
      if (galleryList is Iterable) {
        for (final dynamic img in galleryList) {
          final String imgId = (img?.imgId ?? '').toString().trim();
          final String imgPath = (img?.imgPath ?? '').toString().trim();
    ...
    try {
      ...
      if (localPath.trim().isNotEmpty) return true;
    } catch (_) {}
  ...
  bool _isBuildContextMounted(BuildContext ctx) {
    return ctx is Element && ctx.mounted;
  }

  Future<bool> _openCreatedProductShareOptions(
      Product product, {
        BuildContext? hostContext,
        String? resolvedImageUrl,
      }) async {
    final BuildContext effectiveContext = hostContext ?? context;
    if (!_isBuildContextMounted(effectiveContext)) return false;

    final String imageUrl =
        resolvedImageUrl ?? _resolveCreatedProductShareImageUrl(product);

    log('created product share imageUrl = $imageUrl');

    try {
      await ProductShareOptions.show(
      ...
      log('ProductShareOptions.show failed: $e');
      return false;
    }
  }
  ...
  // DashboardView عندك يوفّر فعلياً:
  // - reloadSwapOpportunitiesAfterProductUpload(String itemId)
  // - goToBottomTab(int bottomIndex)
  // - reloadSwapOpportunities()
  // - refreshSwapOpportunities()
  // - refreshSuggestedSwaps()
  // لذلك لا نعتمد هنا على أسماء تخمينية، ونستخدم الأسماء الموجودة في dashboard_view.dart فقط.
  static const int _swapOpportunitiesBottomTabIndex = 1;

  bool _callDashboardSwapOpportunitiesHooks(String itemId) {
    bool handled = false;

    void tryCall(dynamic state) {
      if (handled) return;

      // الاسم الصحيح الموجود في DashboardView: يغير التاب إلى فرص التبديل
      // ويزود reload nonce لترشيحات المنتج الجديد.
      try {
        state.reloadSwapOpportunitiesAfterProductUpload(itemId);
        handled = true;
        return;
      } catch (_) {}

      // الاسم الصحيح الموجود في DashboardView للتنقل بين bottom tabs.
      try {
        state.goToBottomTab(_swapOpportunitiesBottomTabIndex);
        handled = true;
      } catch (_) {}

      // Reload hooks الموجودة فعلاً في DashboardView. لا تعتبر وحدها انتقالاً
      // مضموناً، لكنها مفيدة بعد goToBottomTab أو عند وجود تبويب مفتوح بالفعل.
      if (handled) {
        try {
          state.reloadSwapOpportunities();
        } catch (_) {}
        try {
          state.refreshSwapOpportunities();
        } catch (_) {}
        try {
          state.refreshSuggestedSwaps();
        } catch (_) {}
        return;
      }
    ...
    try {
      context.visitAncestorElements((Element element) {
        if (element is StatefulElement) {
          tryCall(element.state);
          if (handled) return false;
        }
        return true;
      });
    } catch (e) {
      log('[ITEM_ENTRY_CLOSE] dashboard hook search failed: $e');
    }

    return handled;
  }

  Future<bool> _closeEntryRouteIfPossible({
    required BuildContext routeContext,
    required NavigatorState entryNavigator,
    required NavigatorState rootNavigator,
  }) async {
    try {
      final ModalRoute<dynamic>? currentRoute = ModalRoute.of(routeContext);

      // لو شاشة الإضافة Tab داخل Dashboard، فالـ route غالباً هو أول route
      // في التطبيق. ممنوع نعمل pop هنا حتى لا نقفل Dashboard نفسه.
      if (currentRoute == null || !currentRoute.isActive || currentRoute.isFirst) {
        return false;
      }

      final NavigatorState? ownerNavigator = currentRoute.navigator;

      if (currentRoute.isCurrent) {
        if (ownerNavigator != null && ownerNavigator.canPop()) {
          ownerNavigator.pop(true);
        } else if (entryNavigator.canPop()) {
          entryNavigator.pop(true);
        } else if (rootNavigator.canPop()) {
          rootNavigator.pop(true);
        } else {
          return false;
        }
      } else if (ownerNavigator != null) {
        // لو فوق شاشة الإدخال popup/dialog غير متوقع، نحذف route شاشة الإدخال نفسها.
        ownerNavigator.removeRoute(currentRoute);
      } else {
        return false;
      }
      ...
      log('[ITEM_ENTRY_CLOSE] close route failed: $e');
      return false;
    }
  }

  Future<void> _waitForDashboardToLeaveEntry() async {
    // ندي فرصة للـ Dashboard يعمل setState أو للـ route يتقفل قبل ظهور الشيت.
  ...
  Future<bool> _exitEntryOrDashboardAfterProductUpload({
    required String itemId,
    required NavigatorState entryNavigator,
    required NavigatorState rootNavigator,
    bool tryRoutePop = true,
  }) async {
    bool handledByCallback = false;
    bool handledByDashboardHook = false;
    bool routeClosed = false;

    // 1) المسار الأصح في حال شاشة الإضافة مفتوحة كـ Route من Dashboard:
    // DashboardView يمرر onItemUploaded وفيه يستدعي reloadSwapOpportunitiesAfterProductUpload.
    try {
      widget.onItemUploaded?.call(itemId);
      handledByCallback = widget.onItemUploaded != null;
    } catch (e) {
      log('[ITEM_ENTRY_CLOSE] onItemUploaded failed: $e');
    }

    // 2) لو شاشة الإضافة Tab داخل Dashboard، فالـ Dashboard يكون ancestor
    // ويمكن الوصول للأسماء الصحيحة الموجودة فيه مباشرة.
    handledByDashboardHook = _callDashboardSwapOpportunitiesHooks(itemId);

    // 3) لو شاشة الإضافة Route مستقلة، اقفلها بعد طلب انتقال Dashboard.
    if (tryRoutePop) {
      routeClosed = await _closeEntryRouteIfPossible(
        routeContext: context,
        entryNavigator: entryNavigator,
        rootNavigator: rootNavigator,
      );
    }

    await _waitForDashboardToLeaveEntry();

    final bool handled = handledByCallback || handledByDashboardHook || routeClosed;
    log('[ITEM_ENTRY_CLOSE] handled=$handled callback=$handledByCallback dashboardHook=$handledByDashboardHook routeClosed=$routeClosed itemId=$itemId');
    return handled;
  }
  ...
  Future<void> _showAdminApprovalSheetThenGoHome(Product createdProduct) async {
    ...
    // مهم: جهّز بيانات الشير قبل غلق شاشة الإدخال، لأن local image resolver
    // تابع للـ State الحالي وقد لا يكون آمناً الاعتماد عليه بعد dispose.
    final String resolvedShareImageUrl =
        _resolveCreatedProductShareImageUrl(createdProduct);

    // خزن الـ navigators قبل أي pop. سنستخدم rootNavigator.context بعد انتظار frame.
    final NavigatorState entryNavigator = Navigator.of(context);
    final NavigatorState rootNavigator = Navigator.of(context, rootNavigator: true);

    // المطلوب: بعد إضافة المنتج ورفع الصور، نحاول الخروج من شاشة الإدخال أولاً
    // سواء كانت Route مستقلة أو Tab داخل Dashboard.
    await _exitEntryOrDashboardAfterProductUpload(
    ...
    // استخدم rootNavigator.context بعد محاولة الإغلاق/تغيير التاب، وليس context الفورم.
    final BuildContext sheetHostContext = rootNavigator.context;
    if (!_isBuildContextMounted(sheetHostContext)) return;
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (BuildContext ctx) {
        ...
        Future<void> closeApprovalSheetOnly() async {
        ...
        Future<void> ensureEntryClosedAfterSheet() async {
          // Safety net: لو لأي سبب الشيت ظهر فوق شاشة الإدخال، الضغط على إغلاق
          // أو الرجوع من الشير سيحاول الخروج مرة أخرى بدل ترك الشاشة مفتوحة.
          if (!mounted) return;
        ...
        Future<void> closeAndStayOnDashboard() async {
          await closeApprovalSheetOnly();
          await ensureEntryClosedAfterSheet();
        }

        Future<void> shareThenCloseSheet() async {
          await closeApprovalSheetOnly();

          if (!_isBuildContextMounted(sheetHostContext)) return;

          await _openCreatedProductShareOptions(
            createdProduct,
            hostContext: sheetHostContext,
            resolvedImageUrl: resolvedShareImageUrl,
          );
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                TaapdeelButton(
                  label: 'شارك منتجك على السوشيال ميديا',
                  isPrimary: true,
                  isExpanded: true,
                  onPressed: () async => shareThenCloseSheet(),
                ),
                const SizedBox(height: 12),
                TaapdeelButton(
                  label: 'إغلاق',
                  isPrimary: false,
                  isExpanded: true,
                  onPressed: () async => closeAndStayOnDashboard(),
                ),
              ],
            ),
          ),
