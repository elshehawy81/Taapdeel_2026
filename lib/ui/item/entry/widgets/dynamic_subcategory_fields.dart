...
class _DynamicSubCategoryFieldsState extends State<DynamicSubCategoryFields> {
  ...
  List<Widget> _buildFieldsForSubCategory() {
    ...
    // 👕👟 ملابس وأحذية (عام)
    // =============================
    if (hasAny(<String>[ 'احذيه', 'أحذية', 'حذاء'])) {
      return <Widget>[
       // _sectionTitle('👕👟 ملابس وأحذية'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر البراند'),
        _textField(keyName: 'size', label: 'المقاس', hint: 'مثال: M / 42 / 12Y'),
        if (hasAny(<String>['احذيه', 'أحذية', 'حذاء']))
          _toggle(keyName: 'comfortable_for_walking', label: 'مريح للمشي؟ ✔️'),
      ];
    }
    ...
    if (hasAny(<String>['موبايل', 'موبايلات', 'تابلت', 'tablet', 'mobile'])) {
      return <Widget>[
       // _sectionTitle('📱 موبايلات وتابلت'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر من القائمة'),
        _textField(keyName: 'model', label: 'الموديل'),
        _textField(keyName: 'storage', label: 'السعة', hint: 'مثال: 128GB'),
        _textField(
    ...
    if (hasAny(<String>['كاميرا', 'كاميرات', 'camera'])) {
      return <Widget>[
       // _sectionTitle('📷 كاميرات'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر من القائمة'),
        _textField(keyName: 'model', label: 'الموديل'),
        _textField(keyName: 'lens', label: 'العدسة (اختياري)', hint: 'مثال: 18-55mm'),
      ];
    }
    ...
    if (hasAny(<String>['لاب', 'laptop', 'كمبيوتر', 'computer', 'شاشات', 'monitors'])) {
      return <Widget>[
       // _sectionTitle('💻 أجهزة كمبيوتر / لاب توب / شاشات'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر من القائمة'),
        _textField(keyName: 'model', label: 'الموديل (اختياري)'),
        _textField(keyName: 'specs', label: 'المواصفات الأساسية', hint: 'CPU/RAM/SSD/GPU'),
      ];
    }
    ...
    if (hasAny(<String>['اجهزه ذكيه', 'أجهزة ذكية', 'smart'])) {
      return <Widget>[
        ...
        _standardPickerField(
          ...
          label: 'النوع',
          hint: 'اختر',
          items: const <String>['ساعة', 'سوار', 'سماعة', 'أخرى'],
        ),
        _standardPickerField(
          keyName: 'compatible_with',
          label: 'متوافق مع',
          hint: 'اختر',
          items: const <String>['Android', 'iOS', 'Android/iOS'],
        ),
      ];
    }
    ...
    if (hasAny(<String>['السيارات', 'سياره', 'سيارة'])) {
      return <Widget>[
       // _sectionTitle('🚗 السيارات'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر من القائمة'),
        _standardPickerField(
          keyName: 'car_item_type',
          label: 'النوع',
          hint: 'اختر',
          items: const <String>['إكسسوار', 'قطعة'],
        ),
      ];
    }
      ...
      return <Widget>[
       // _sectionTitle('🧰 تفاصيل إضافية'),
        ...
        _textField(
          keyName: 'type',
          label: 'النوع',
          hint: 'مثال: ماكينة / خيط / ألوان / عدة',
        ),
      ];
    }
    ...
    if (hasAny(<String>[
      ...
      final List<Widget> fields = <Widget>[
       // _sectionTitle('🏠 تفاصيل إضافية'),
        _brandField(keyName: 'brand', label: 'البراند', hint: 'اختر من القائمة'),
        _textField(keyName: 'type', label: 'النوع', hint: 'مثال: خلاط / كيس مخدة / ديكور'),
      ];
