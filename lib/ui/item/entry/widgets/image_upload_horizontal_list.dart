...
class ImageUploadHorizontalListState extends State<ImageUploadHorizontalList>
  ...
  int _slotCount() {
    final int galleryCount = widget.galleryImagePath.length;
    final int cameraCount = widget.cameraImagePath.length;
    final int selectedCount = widget.selectedImageList?.length ?? galleryCount;

    return math.max(
      1,
      math.min(galleryCount, math.min(cameraCount, selectedCount)),
    );
  }
  ...
  /// ✅ تصوير صورة جديدة بالكاميرا (صورة واحدة فقط)
  Future<List<XFile>> _pickProfessionalSingleImageFromCamera() async {
    try {
      final XFile? single = await _picker.pickImage(
  ...
  /// ✅ صورة جديدة من الكاميرا لسلوت واحد
  Future<void> loadSingleImageFromCamera(int index) async {
    final List<XFile> resultList = await _pickProfessionalSingleImageFromCamera();
  ...
  /// ✅ Bottom sheet لاختيار مصدر الصورة: كاميرا أو معرض
  Future<void> _showImageSourceSheet(int slotIndex) async {
              ...
              children: <Widget>[
                ...
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    await loadSingleImageFromCamera(slotIndex);
                  },
                ),
                ListTile(
  ...
  Future<void> _openPickerForSlot(int slotIndex) async {
    ...
    // ✅ نعرض اختيار (كاميرا / معرض) للمستخدم بدل الذهاب للمعرض مباشرة
    await _showImageSourceSheet(slotIndex);
  }
