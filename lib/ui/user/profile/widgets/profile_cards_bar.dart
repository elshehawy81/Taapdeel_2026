...
class ProfileHorizontalCardsBarState extends State<ProfileHorizontalCardsBar> {
  ...
  Widget build(BuildContext context) {
    ...
    // منطق إظهار الكروت:
    // family    → دايماً ظاهر
    // !countReady (NOACTION) → مخفي تماماً (لسه ما اتطلبش من الـ API)
    // isLoading → ظاهر (عشان المستخدم يشوف spinner)
    // count > 0 → ظاهر (فيه منتجات)
    // count = 0 + showWhenEmpty → ظاهر (زي pending — يظهر "لا يوجد" بدل الاختفاء)
    // count = 0 + !showWhenEmpty → مخفي (sold/rejected/disabled لو فاضيين)
    final items = raw.where((it) {
      if (it.type == ProfileTabType.family) return true;
