...
class _WishItemEntryViewState extends State<WishItemEntryView>
        ...
        children: <Widget>[
          Row(
            ...
            children: <Widget>[
              ...
              Expanded(
                child: Text(
                  isArabic ? 'عناوين مقترحة للحدوتة' : 'Suggested story titles',
                  textDirection: TextDirection.rtl,
        ...
        return TaapdeelGlassBottomSheet(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                isArabic ? 'تمت إضافة الحدوتة' : 'Story wish added',
              ...
              Text(
                isArabic
                    ? 'تم حفظ الحاجة اللي نفسك فيها بنجاح.\nتقدر تشاركها كحدوتة لطيفة أو كمنتج مطلوب.'
              ...
              if (canShare) ...<Widget>[
                TaapdeelButton(
                  label: isArabic ? 'شارك كحدوتة' : 'Share as story',
                  isPrimary: true,
                  isExpanded: true,
                  onPressed: () async {
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _ThemedInputField(
            ...
            label: isArabic ? 'عنوان الحدوتة (5 كلمات)' : 'Story title (5 words)',
            hint: isArabic ? 'مثال: الفستان اللي محتاجاه بجد' : 'Example: The dress I really need',
            theme: theme,
            maxLines: 1,
            inputFormatters: const <TextInputFormatter>[
              _MaxWordsTextInputFormatter(5),
            ],
          ),
          const SizedBox(height: 8),
          ...
          _ThemedInputField(
            ...
            label: isArabic ? 'الحدوتة (سطرين)' : 'Story (2 lines)',
            hint: isArabic
                ? 'اكتب الحدوتة في سطرين فقط…'
  ...
  Future<void> _submitWishItem() async {
    ...
    if (loginUserId.isEmpty || loginUserId == 'null') {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isArabic ? 'يجب تسجيل الدخول قبل حفظ الحدوتة.' : 'Please login before saving the wish.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
      ...
      if (_isThemedWishEntry) {
        if (_storyCardTitleController.text.trim().isEmpty) {
          showDialog<dynamic>(
            context: context,
            builder: (_) => ErrorDialog(
              message: isArabic ? 'اكتب عنوان الحدوتة.' : 'Please enter the story title.',
            ),
          );
          return;
        }
        if (_wordCount(_storyCardTitleController.text) > 5) {
          showDialog<dynamic>(
            context: context,
            builder: (_) => ErrorDialog(
              message: isArabic ? 'عنوان الحدوتة يجب ألا يزيد عن 5 كلمات.' : 'Story title must not exceed 5 words.',
            ),
          );
          return;
        }
        if (_hawadeetStoryController.text.trim().isEmpty) {
          showDialog<dynamic>(
            context: context,
            builder: (_) => ErrorDialog(
              message: isArabic ? 'اكتب الحدوتة في سطرين.' : 'Please write the two-line story.',
            ),
          );
          return;
        }
        ...
        if (storyLines > 2) {
          showDialog<dynamic>(
            context: context,
            builder: (_) => ErrorDialog(
              message: isArabic ? 'الحدوتة يجب ألا تزيد عن سطرين.' : 'Story must not exceed two lines.',
            ),
          );
          return;
        }
        ...
        child: Consumer<UserProvider>(
          builder: (BuildContext context, UserProvider provider, Widget? child) {
            ...
            final String bottomLabel = _submittingWish
                ...
                : (isArabic ? 'حفظ الحدوتة' : 'Save wish');
